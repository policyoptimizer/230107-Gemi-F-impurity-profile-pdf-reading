...
def create_heatmap(df):
    ...
    # 조건에 맞는 셀들을 강조하기 위한 패치 추가
    for i in range(final_result_df.shape[0]):
        for j in range(final_result_df.shape[1]):
            rrt = final_result_df.index[i]
            value = final_result_df.iloc[i, j]
            if (rrt > 1) and (0.05 <= value <= 10):
                # 셀에 노란색 테두리 추가
                rect = patches.Rectangle((j, i), 1, 1, fill=False, edgecolor='yellow', linewidth=2)
                ax.add_patch(rect)
