# Name 칼럼에 값이 없어도 최종 df 에 포함되도록 함
# Name 칼럼이 공란이면 0 이 기입되어야 하는데 3, 3, 35, 7, 등 특정한 값이 입력됨
# 그나마 현재까지는 이게 베스트임
...
# PDF 파일이 있는 폴더
pdf_folder = 'D:/#.Secure Work Folder/BIG/Project/23~24Y/230106 Gemi-F SCMQuality 관리 시스템 구축/impurity profile pdf reading/pdf/DP72/합한거/'
        ...
        with pdfplumber.open(pdf_path) as pdf:
            for page in pdf.pages:
                ...
                if peak_results_start != -1:
                    ...
                    # Peak Results 데이터 파싱
                    peaks = re.findall(r'(\d+)\s+([\w-]*)\s*([\d.]+)\s+(\d+)\s+(\d+)\s+([\d.]+)\s+(\d+)\s+([A-Z]+)', peak_results_text)
                    for peak in peaks:
                        new_row = pd.DataFrame({
                            'Sample Name': [sample_name],
                            'Peak Number': [peak[0]],
                            'Name': [peak[1] if peak[1] else '0'],  # Name이 없는 경우 '0'으로 처리
                            'RT': [peak[2]],
                            'Area': [peak[3]],
                            'Height': [peak[4]],
                            '% Area': [peak[5]],
